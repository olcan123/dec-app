<template>
  <section class="space-y-6 p-6">
    <PagesHeader
      :title="pageTitle"
      :description="
        declaration ? 'Beyan detaylarını görüntüleyin' : 'Beyan yükleniyor...'
      "
      action-label="Geri"
    ...
    <div
      ...
      class="rounded-2xl border border-rose-200 bg-white/80 p-6 text-sm text-rose-600"
    >
      Hata: {{ errorValue }}
    </div>
    <div
    ...
    <div v-else class="space-y-6">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h3 class="text-base font-semibold leading-6 text-slate-800 mb-4">
            Genel Bilgiler
          </h3>
          <div class="space-y-2 border-t border-slate-200 pt-4">
            ...
            >
              ...
              <dd class="text-sm text-slate-900 font-medium">
                {{ declaration.id }}
              </dd>
            </div>
            <div
            ...
            >
              ...
              <dd class="text-sm text-slate-900 font-medium">
                {{ customerTitle || declaration.customerId }}
              </dd>
            </div>
            <div
              class="flex justify-between items-center py-2 border-b border-slate-100"
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div class="space-y-2 border-t border-slate-200 pt-4">
            ...
            >
              ...
              <dd class="text-sm text-slate-900 font-medium">
                {{ formatCurrency(declaration.paidAmount) }}
              </dd>
            </div>
            <div
              class="flex justify-between items-center py-2 border-b border-slate-100"
            >
              ...
              <dd class="text-sm text-slate-900 font-medium">
                {{ formatCurrency(declaration.lateFee) }}
              </dd>
            </div>
            <div
              class="flex justify-between items-center py-2 border-b border-slate-100"
      ...
      <div class="mt-6">
        ...
        <div
          v-if="notesLoading"
          class="rounded-lg border border-slate-200 bg-white/80 p-4 text-sm text-slate-500"
        >
          Notlar yükleniyor...
        </div>
        <div
          v-else-if="notesError"
          class="rounded-lg border border-rose-200 bg-white/80 p-4 text-sm text-rose-600"
        >
          Notlar yüklenemedi: {{ notesError }}
        </div>
        <div v-else>
          <div
            v-if="!notes.length"
          ...
          >
            Bu beyana ait not bulunamadı.
          </div>
          <ul v-else class="space-y-4">
            <li
              v-for="note in notes"
              :key="note.id"
              class="flex gap-4 rounded-lg border border-slate-100 bg-white p-4 shadow-sm"
            >
              <div class="flex-shrink-0">
                <div
                  class="h-10 w-10 rounded-full bg-indigo-50 text-indigo-700 flex items-center justify-center font-semibold text-lg"
                >
                  {{
                    note.createdBy
                      ? String(note.createdBy).charAt(0).toUpperCase()
                      : note.title
                      ? String(note.title).charAt(0).toUpperCase()
                      : "N"
                  }}
                </div>
              </div>
              <div class="flex-1 min-w-0">
                <div class="flex items-start justify-between gap-3">
                  <div>
                    <div class="text-sm font-semibold text-slate-800 truncate">
                      {{ note.title || `Not #${note.id}` }}
                    </div>
                    <div
                      v-if="note.createdBy"
                      class="text-xs text-slate-500 mt-0.5"
                    >
                      Oluşturan: {{ note.createdBy }}
                    </div>
                  </div>
                ...
                <div
                  class="mt-2 text-sm text-slate-700 whitespace-pre-wrap break-words"
                >
                  {{ note.content }}
                </div>
                <div
...
import { statusOptions, priorityOptions } from '~/utils/declarations/form.js';

// Status label gösterimi
const statusLabel = computed(() => {
  const status = declarationRef.value?.status;
  const found = statusOptions.find(opt => opt.value === status);
  return found ? found.label : status;
});

// Priority label gösterimi
const priorityLabel = computed(() => {
  const priority = declarationRef.value?.priority;
...
const customerTitle = computed(() => {
  if (!declarationRef.value?.customerId)
    return `ID: ${declarationRef.value?.customerId ?? "?"}`;
  const c = (customers.value || []).find(
    (cu) => String(cu.id) === String(declarationRef.value.customerId)
  );
  return c ? c.title : `ID: ${declarationRef.value.customerId}`;
});

const typeName = computed(() => {
  if (!declarationRef.value?.typeId)
    return `ID: ${declarationRef.value?.typeId ?? "?"}`;
  const types = declarationStore.declarationTypes || [];
  const t = (types || []).find(
    (tt) => String(tt.id) === String(declarationRef.value.typeId)
  );
...
const formattedDueDate = computed(() =>
  formatDateInternal(declarationRef.value?.dueDate)
);
const formattedCompletionDate = computed(() =>
  formatDateInternal(declarationRef.value?.completionDate)
);
...
const pageTitle = computed(() => {
  ...
  return `Beyan ${customerTitle.value} #${declarationRef.value.periodName} Detayı`;
});
